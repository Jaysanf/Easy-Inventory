...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for(int i = 0; i < height ; i++)
    {
        for(int j = 0; j < width ; j++)
        {
            ...
            image_dump[i][j].rgbtRed = (int) round((float)(average.rgbtRed / counter));
            image_dump[i][j].rgbtGreen = (int) round((float)(average.rgbtGreen / counter));
            image_dump[i][j].rgbtBlue = (int) round((float)(average.rgbtBlue / counter));
        }
    }
