...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        if (cyclecheck(winner_i, loser_i, i))
        {    
...
bool cyclecheck(int winner, int loser, int j)
{
    // if we end up in a situtation like Alice - Alice it means there is a cylce so return false
    if (winner == loser)
    {
        return false;
    }
    //loop that goes through all pairs before the pair winner loser
    for (int i = j; j <= 0; j--)
    {
        if (winner == pairs[i].loser)
        {
            return cyclecheck(pairs[i].winner, loser, i - 1);
        }
    }
    // actual cycle check
