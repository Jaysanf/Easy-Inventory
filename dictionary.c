...
bool check(const char *word)
{
    ...
    char *word_nocap = malloc(strlen(word) + 1);
    ...
    for (int i = 0; i < word_nocap[i]; i++)
    {
        word_nocap[i] = tolower(word_nocap[i]);
    }
    ...
    while (1)
    {
        ...
        else if (strcmp(current_node->word, word_nocap) == 0)
        {
            free(word_nocap);
            return true;
        }
